...
module P3 = struct
  ...
  let process_prods prods = (
    ...
    let f1 prod m = (
      ...
      m')
    in
    let pprods = Prelude.itlist f1 prods Map_symbol_int_int.empty in
    let f1 prod m = (
      let (itm,k) = prod in
      ...
      | TM tm -> (
        ...
        m'))
    in
    let tmpprods = Prelude.itlist f1 prods Map_term_int_int.empty in
    (pprods,tmpprods))
  ...
  let p3_run_parser p txt = (
    let prods = earley_prods p txt in
    let (pprods,tmpprods) = process_prods prods in
    earley_actions p (pprods,tmpprods) txt (0,String.length txt))
